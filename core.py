...
def new_req(user_id, request):
    ...
    cur.execute("INSERT INTO requests (`user_id`, `req_status`) VALUES (%s, 'waiting')", (user_id,))

    # Получить айди добавленного запроса
    req_id = cur.lastrowid

    # Обновить запись, чтобы задать req_id
    cur.execute("UPDATE requests SET req_id = %s WHERE id = %s", (req_id, req_id))

    dt = datetime.datetime.now()
    date_now = dt.strftime('%Y-%m-%d %H:%M:%S')

    # Добавить сообщение для запроса
    cur.execute("INSERT INTO messages (`req_id`, `message`, `user_status`, `date`) VALUES (%s, %s, 'user', %s)", (req_id, request, date_now))
...
def get_req_status(req_id):
    ...
    if result is None:
        return None  # или верните значение по умолчанию, например, 'not_found'
    else:
        return result[0]
